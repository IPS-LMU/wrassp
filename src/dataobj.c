...
dobj2AsspDataObj (DOBJ * data)
{
  ...
  startRec, endRec, trackFormats, finfo, genericVars;
  DDESC *desc = NULL;
...
{
 SEXP ans, var, names, varNames, value;
 ...
 for (i = 0, genVar=&(dop->meta); genVar != NULL; genVar = genVar->next, i++) {}
     if (i == 0) {
      UNPROTECT(1);
      return(R_NilValue);
  }
  ...
  for (i=0, genVar=&(dop->meta); genVar!=NULL; i++, genVar=genVar->next) {
...
{ 
 ...
 size_t numFields = -1;
 SEXP attr, tracks, formats, track, var, varNames;
 SSFFST *ssff_types;
 TSSFF_Generic *genVar;
 KDTAB * entry;
 char * format;
    ...
    if (!isNull(attr)) {
        for (i = 0; i < LENGTH(attr); i++) {
            ...
            for (ssff_types = SSFF_TYPES; ssff_types->type != SSFF_UNDEF; ssff_types++) {
                ...
                if ( strncmp( format, ssff_types->ident, strlen( ssff_types->ident) ) == 0 ) 
                    break;
            }
            if (ssff_types->type == SSFF_UNDEF)
                error("Invalid type for SSFF generic variable.");
            if (FIRST) {
                ...
                if (genVar == NULL)
                    error("Unable to add Generic Variable (%s).", getAsspMsg(asspMsgNum));
            }
            FIRST = 0;
            ...
            switch (genVar->type) {
                case SSFF_CHAR:
                case SSFF_BYTE:
                    genVar->data = strdup( CHAR( STRING_ELT( VECTOR_ELT( var, 0), 0)));
                    break;
                case SSFF_SHORT:
                case SSFF_LONG:
                case SSFF_FLOAT:
