...
performAssp (SEXP args)
{
   ...
   /* output extension might still be unset. For afdiff and 
      for spectral analysis we need to take special care. In other cases it 
      would be weird to get here but we can safely use the default. */
   if (strcmp (ext, "") == 0)
   {
   ...
   /* in memory only works for single input, not for multiple input
      so, if toFile is false but there are multiple inputs
      set toFile to true */
   ...
   for (i = 0; i < length (inputs); i++)
   {
	 ...
	 error ("%s (%s)", getAsspMsg (asspMsgNum), strdup(name));
      ...
      if (toFile)
      {
	 /* in toFile mode, all DOBJs are written to file
	    we will later return the number of successful analyses */
